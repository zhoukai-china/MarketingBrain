...
// “主选题：”“标题：”属于正文；只有带中文序号的十个栏目才能开启新卡片。
const DELIVERY_SECTIONS = [
  { title: "选题策划", pattern: /^一[、.．]\s*(?:选题(?:策划)?|主题)(?:\s.*|[：:].*)?$/ },
  { title: "口播逐字稿", pattern: /^二[、.．]\s*(?:可直接发布的文案|口播(?:逐字稿|文案)?|文案)(?:\s.*|[：:].*|[（(].*)?$/ },
  { title: "访谈话术", pattern: /^三[、.．]\s*(?:访谈|采访)(?:内容[·・])?(?:提问)?话术(?:\s.*|[：:].*)?$/ },
  { title: "拍摄脚本", pattern: /^四[、.．]\s*(?:可直接拍摄的脚本[：:]?\s*)?拍摄脚本(?:\s.*|[：:].*)?$/ },
  { title: "拍摄注意事项", pattern: /^五[、.．]\s*拍摄注意事项(?:\s.*|[：:].*)?$/ },
  { title: "剪辑EDL文件", pattern: /^六[、.．]\s*剪辑\s*EDL(?:文件)?(?:\s.*|[：:].*)?$/i },
  { title: "发布标题与话题标签", pattern: /^七[、.．]\s*发布标题(?:与话题(?:标签)?|话题(?:标签)?)?(?:\s.*|[：:].*)?$/ },
  { title: "最佳发布时间", pattern: /^八[、.．]\s*(?:最佳)?发布时间(?:\s.*|[：:].*)?$/ },
  { title: "评论区引导话术", pattern: /^九[、.．]\s*评论区(?:引导)?话术(?:\s.*|[：:].*)?$/ },
  { title: "投流建议", pattern: /^十[、.．]\s*投流建议(?:\s.*|[：:].*)?$/ }
];
...
function normalizeDeliveryHeadingContent(content: string): string {
  ...
  return inlineFixed.split(/\r?\n/).flatMap((rawLine) => {
    ...
    const ordinal = ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十"][DELIVERY_SECTIONS.indexOf(definition)];
    const canonicalHeading = `${ordinal}、${definition.title}`;
    const headingMatch = clean.match(/^([一二三四五六七八九十][、.．][^：:]*)(?:[：:]\s*(.*))?$/);
