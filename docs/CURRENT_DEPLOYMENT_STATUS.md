...
- **兰琪生产授权现状（实测 2026-09-10 21:07，只读）**：`apps/api/src/server.ts:163` 给 `/lanqi` 作用域挂了 `requireProductEntitlement("lanqi")`，而生产库 `TenantProductEntitlement` 分布为 `takeaway|active 190`、`founder-ip|active 190`、`beauty-industry|active 6 / revoked 7`，**没有任何 `lanqi` 记录**；`InviteCode.productCode` 分布为 `<null> 129`、`beauty-industry 7`，**没有任何 `lanqi` 邀请码**，`LanqiReferral` 0 行；`LanqiMomentDraft` / `LanqiStoreProfile` / `LanqiStoreGoal` 均 0 行。叠加 `apps/web/src/pages/LoginPage.tsx:193`「产品入口恒需邀请码」，结论是：**本次发布已上线，但生产目前还进不了兰琪**——`/os-v2/login/lanqi` 卡在产品邀请码，平台主入口（开放注册）注册出来的租户不带 `lanqi` 授权，`/lanqi/*` API 会 403 `product_entitlement_missing`。这属于权限/数据变更，候选修法见 `docs/agents/lanqi-beauty/STATUS.md`，**需用户拍板后再执行**。
