...
- 2026-09-14（总调度7）：确立目标结构＝1 个公共平台代码区＋每个智能体各自一个代码区；平台少动、向后兼容、改动串行，智能体可并行，先做真实产品再抽公共能力；用户只与总调度沟通需求，由总调度判定公共/单智能体并派发。进展：兰琪 LQ-28 已收口（测试+生产上线）；「平台底座抽取」完成第①②批，第③批已完成（server.ts 产品路由抽到 `apps/api/src/products/register.ts`、main.tsx 兰琪路由抽到 `apps/web/src/routes/lanqi.tsx`，行为不变、qa:fast + web build 全绿）；第④批待推进，暂不做平台发布。
