...
| 项 | 状态 | 等什么 |
| --- | --- | --- |
| **推荐有礼（PLAT-28 第②批）** | 内测通过、**两环境已关闭**（2026-09-16） | 活动窗**暂定 10.1–10.7**（左闭右开 ⇒ end 写 `2026-10-08T00:00:00+08:00`）。等用户说「启动」→ `node scripts/enable-referral-campaign.mjs --apply --start … --end …`。⚠️ 2026-09-16 发现生产 DB 覆盖位曾被置 true 并发出 1 笔 100 积分（见 `docs/BUG_REGRESSIONS.md` QA-20260916-005）——**改这条配置后必须回读生效值**，不能只看 env 文件 |
...
| **汽配信息网补交付物** | 等用户 | 客户原 IP 定位报告不可找回（留存能力上线前）；已全额退还 157 积分。要补一份新的 Word 全案，需用户把该客户当时的 4 项填写内容发来（不占客户账号、不扣积分） |
