...
- **开放项清单**（平台侧）：**PLAT-48 市场合伙人分销系统 + PLAT-49 自助生成专属链接**已在分支 `codex/plat48-market-partner` 本地完成、真实 PG DB smoke 全绿（2026-09-18），待上线（迁移 `001`/`002`/`003` 需部署）；PLAT-26 分润冻结已由 PLAT-48 解除（老板拍板 20% 统一口径）；PLAT-28 第②③批代码完成但**活动未开**（见下）；PLAT-32 剩 `registerAuthRoutes` 拆分；PLAT-44 遗留兰琪/美业自有页面的 `/my-ai` 入口、美业 `beauty-directory-browser-e2e`、`MyAiPage` 去留。产品侧：美业 BY-55 等用户提供非生产 Ark 账号 + 已授权素材 + 预算核验（外部 PAUSED）；创始人 IP 扩展与外卖增长均无进行中编码任务。
...
| 项 | 状态 | 等什么 |
| --- | --- | --- |
| **推荐有礼（PLAT-28 第②批）** | 内测通过、**两环境已关闭**（2026-09-16） | 活动窗**暂定 10.1–10.7**（左闭右开 ⇒ end 写 `2026-10-08T00:00:00+08:00`）。等用户说「启动」→ `node scripts/enable-referral-campaign.mjs --apply --start … --end …`。⚠️ 2026-09-16 发现生产 DB 覆盖位曾被置 true 并发出 1 笔 100 积分（**用户已确认「可以保留」，不回收**；见 `docs/BUG_REGRESSIONS.md` QA-20260916-005）——**改这条配置后必须回读生效值**，不能只看 env 文件 |
| ~~**文生视频模型选型**~~ | **已拍板：A** | 用户 2026-09-16「选 A：用百炼现成的 t2v」→ 已按秒 ×2 = 12 积分/秒接通（`20260916-plat47`），4 个旧包价作废 |
| ~~**视觉倍数**~~ | **已拍板：25×** | 视觉 ¥0.5/次（= 25 × ¥0.02 估算成本）；**无真实视觉消耗不收费**（纯文字 PDF 不再保底扣 40）。已发 `20260916-plat46` |
| ~~**按成本计费封顶**~~ | **已拍板：不加** | 用户「不用加封顶——按我们要的利润，该是多少就多少」 |
| ~~**5 类无样本 SKU 取证**~~ | **已拍板：上架后按倍数算** | 不做上架前取样；上架后测出成本按对应倍数计费即可 |
| **PLAT-23 换算口径收尾** | 部分完成 | 用户已给「1 元=20 积分 + 各倍数」口径；货架按成本口径已接线（`BILLING_COST_BASED_SKUS=*`，`20260916-plat45/47`）；剩「冷门 SKU 参考价」随后续样本再调 |
| ~~**汽配信息网补交付物**~~ | **已闭环（2026-09-16）** | 用户 2026-09-16 答复「这件事处理完了」：原报告不可找回（留存能力上线前）已全额退还 157 积分，客户侧由用户自行交付，平台无需再补 Word。该类事故的根治能力已上线（交付物服务端留存 7 天 + `/market/me/deliverables`），后续同类情况可直接找回而非只能退款 |
| **上传文件作为生成参考（IP 定位现场「Excel 读不出来」）** | **待用户拍板 A / B** | 现场：IP 定位里客户传了 `.xlsx`，智能体只记文件名、最后请他改用文字补充。**已核查：解析链路早已上线**——`POST /media/analyze` 支持 PDF/DOCX/XLSX/XLS/CSV/TXT/MD/JSON/图片/音视频，其中**纯文档（Excel/Word/PDF 文本/CSV）不调模型、不收费**，图片与扫描件走视觉（按次计费 + 失败退款）；今天只有**视频复盘**在用（前端 `isVidrev` 判断），其它智能体只记文件名。**A 方案（放开给全部智能体）≈0.5 天**（前端放开判断 + 每类提示 + 确认卡显示已带入文件），若要图片/扫描件则先定计费口径（≈1–2 天）；**B 方案（只文字+语音）**注意**不能全局关**——视频复盘的核心输入就是后台导出 Excel，只能关不依赖文件的智能体入口。稳定性风险不在「读不出来」，而在「读进来怎么进 prompt」（大表/长 PDF 挤上下文 → 漏章/编数），建议服务端先做结构化摘要 + 缺字段一律标待补 |
| **PLAT-26 市场合伙人分润** | **已由 PLAT-48/49 取代** | 2026-09-18 老板拍板 20% 统一口径，冻结解除；PLAT-48/49 本地完成、DB smoke 全绿，待上线 |
