...
- **开放项清单**（平台侧）：**PLAT-26 市场合伙人分润已按用户 2026-09-16 指示冻结**（不再催口径，等用户启动）；PLAT-28 第②③批代码完成但**活动未开**（见下）；PLAT-32 剩 `registerAuthRoutes` 拆分；PLAT-44 遗留兰琪/美业自有页面的 `/my-ai` 入口、美业 `beauty-directory-browser-e2e`、`MyAiPage` 去留。产品侧：美业 BY-55 等用户提供非生产 Ark 账号 + 已授权素材 + 预算核验（外部 PAUSED）；创始人 IP 扩展与外卖增长均无进行中编码任务。
