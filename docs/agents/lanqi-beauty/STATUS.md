# 兰琪美业经营增长智能体状态
> **2026-09-18 LQ-35 门店读取失败不再伪装成「还在加载」（已上测试实例 + 生产）**：老板在「公域获客 → 美业文案十件套」点生成只看到「门店信息还在加载，请稍后再试一次。」——取证确认 `GET /os-v2/api/lanqi/stores` 实际 **403 / 175 字节**（`code=product_entitlement_missing|expired`，即该租户没有生效中的兰琪权益），前端把 403 吞成「门店为空」再用「加载中」兜底。修复：两页（`LanqiAcquireCopyKitPage.tsx` / `LanqiAcquireVideoPage.tsx`）改用既有 `useLanqiStoreGate` + `LanqiStoreGateBanner`，删除「门店信息还在加载」，读门店失败按真实 `code` 分流提示与 CTA。绿证：契约冒烟 144/0、门店判定 44/0、`qa:fast` exit 0；生产线上「门店信息还在加载」=0、新口径提示各 1、`referenced_missing=0`、health/ready 200、err 日志 No entries。**真人验收（2026-09-18，老板本人）已通过**：老板在生产微信真实登录，进入「公域获客 → 美业文案十件套」正常、未再出现「门店信息还在加载」，点生成正常出结果、无问题，本卡归档。详见 `docs/BUG_REGRESSIONS.md` QA-20260918-001 与任务卡 `tasks/LQ-35-门店读取失败被说成还在加载.md`。
> **2026-09-17/18 取消兰琪邀请码制度（已上线测试 + 生产 + 真人验收通过，本卡闭环）**：产品入口不再强制邀请码；兰琪登录去掉独立邀请码表单，直接微信授权 + 门店资料开通。**真人验收（2026-09-18，老板本人）**：在生产用微信真实登录兰琪，确认「不用填邀请码就能开通/进入」端到端成立。详见 `docs/CURRENT_DEPLOYMENT_STATUS.md` 顶部 `20260917-lq-invite-removal`。
> **2026-09-17 分支合流**：`codex/lq34-wallet` 已 fast-forward 合并回 `main`（`main` = `cf169d3`），防止未来从 `main` 发布把生产打回 `plat67`。
> **2026-09-17 早 LQ-34 生产收口（迁移 + 发布 + 只读核对全部完成；只剩老板门店实跑）**：按老板 2026-09-16 晚确认的顺序执行完毕。**① 生产历史额度迁移（2026-09-17 06:42 CST，库 `baolu_os_v2` / schema `public`）**：迁移前快照重出一份可信版本（`pre-state-snapshot-v2.txt`；第一版嵌套引号被 shell 吃掉、两段查询假 0 行，已弃用），把口径钉死——两条候选 `cmt6idd1c04v62hgb86gvh7pz` **300** / `cmt6wlw7w0517pou3005wvk8o` **208** = **2 个账户 / 508 积分**，全库有余额 **201 / 2,001,160,483**，`Wallet` 合计 **25 个 / paid 11,048 / bonus 1,100**，迁移标记 **0**，另存全库 dump（8,085,309 B）兜底；`--apply --backup /opt/baolu-backups/lq34-migration-prod-20260917-0641/` **exit 0**，迁移前 CSV 已出（表头 + 2 条）。迁移后对账：`WalletLedger` `lanqi-migrate:%` **2 行 / 508**（`type=admin`、`bucket=paid`、无重复 refRequestId）、`CreditTransaction` `adjust` **2 行 / 508** 留痕、两条候选 `CreditAccount` **归 0 且行保留**、owner 钱包 **300 / 208**、**非兰琪 199 个 / 2,001,159,975 一分未动**、`Wallet` 变为 **26 个 / paid 11,556**；随后 dry-run **计划为空**（幂等）。**② 生产发布（06:44–06:47，发布 id `20260916-lq34-wallet-prod1`）**：用的就是测试实例验证过的**同一份归档**（1,584 文件 / 9,978,440 B / sha256 `2d4fa183…`）；发布**前**先做「归档 vs 线上源码」逐文件 sha256 比对（**1558 一致 / 6 新增 / 20 差异且全部是 LQ-34 自身改动**）确认不会把生产 `plat67` 回退；结果 **`DEPLOY_OK` + `VERIFY_OK`（0 FAIL）**、`No pending migrations`、重启后 err 日志 **0 条**、dist 重建于 06:46 且含 `chargeLanqiWallet` / `precheckLanqiWallet` / `refundLanqiWallet`、`health/ready 200`、生产匿名 `POST /os-v2/api/lanqi/acquire/copy-kit` **401 `login_required`**。**③ 只读核对老板门店余额与流水**：两个兰琪门店 `industry=美业`、`lanqi` 权益 `active`、`CreditAccount.balance=0`（行保留）、owner 钱包 **300 / 208**，owner 侧钱包流水只有两条迁移入账（`type=admin` / `bucket=paid` / `source=lanqi_credit_migration:<tenantId>`）。**④ 未做（如实说）：老板门店实跑一次低价生成**——需真人微信扫码登录，Codex 不能代持（同 LQ-22 / LQ-28 / LQ-29 既有边界）；建议老板登录后在兰琪「公域获客 → 美业文案十件套」（`/lanqi/acquire/copy-kit`，**40 积分/次**）跑一次，再用同款只读 SQL 核对「钱包扣 40 + 兰琪余额同步 + 思潼货架同一本账」。回滚＝`/opt/baolu-backups/20260916-lq34-wallet-prod1-before-baolu-os-v2/`（198M，app 备份 + 发布前库 dump + env + service + dist 哈希）+ `systemctl restart baolu-os-v2`；迁移反做＝CLI `--revert <tenantId,...>`（钱包 paid 不足时 fail closed）。证据文件：`/opt/baolu-backups/lq34-migration-prod-20260917-0641/{pre-state-snapshot-v2.txt,lanqi-credit-migration-1789598547691.csv,post-state-reconcile.txt,post-release-verify.txt,db-before-lanqi-wallet-migration.sql.gz}`。
> **2026-09-16 夜 LQ-34 断点（测试实例已发布 + 历史额度迁移已执行；生产迁移与发布未做）**：老板已确认「迁移口径（只迁兰琪租户）+ 候选清单（生产 2 个账户 / 508 积分）」并授权按「**先测试实例、再生产**」执行，发布顺序定为「**生产迁移 → 生产发布 → 只读核对老板门店余额与流水 → 老板门店实跑一次低价生成**」。**已做**：① 用 `ff88530` 全量包重发测试实例 `20260916-lq34-wallet-test1`（`DEPLOY_OK` + `VERIFY_OK`，health/ready 200）——原因见下；② 测试实例迁移 `--apply --backup /opt/baolu-backups/lq34-migration-test-20260916-2102/`，迁移前 CSV 43 行，实迁 **43 个账户 / 12,520 积分**（比当天 dry-run 的 42 / 12,420 多一条 100 积分新租户），迁移后对账：`WalletLedger` **43 行 / 12,520**（`type=admin`、`bucket=paid`）、`CreditTransaction` `adjust` **43 行 / 12,520** 留痕、已迁账户余额全 0 且行保留、**非兰琪 26 个 / 7,800 一分未动**、owner 钱包 paid ≥ 迁入额；重复 `--apply` 仍 43 / 12,520（**幂等**）。**新增一条发布纪律（QA-20260916-016）**：白天那份测试实例全量包（`lq34-wallet-full-20260916-1335`）被平台线 `plat67` 在 20:42 叠加发布覆盖了 `apps/api/src/services/sitong-wallet.ts`（缺 `withWalletTransaction` / `WalletDb`），首次 apply 直接崩 `withWalletTransaction is not a function`（**exit 1、未写任何数据**）；叠加发布是静默的混合构建，**跑任何数据操作前必须按关键文件 sha256 核对服务器与本地一致**。**尚未做**：生产 dry-run 复核、生产额度迁移、生产发布、老板门店实跑低价生成；**生产仍跑 `plat67`（未含 LQ-34）**。另记一个对账口径陷阱：测试实例与生产**共用同一个物理库 `baolu_os_v2`**，靠 PostgreSQL schema `lanqi_test` 隔离，用 psql 对账**必须先 `SET search_path TO lanqi_test`**，否则读到的是生产数据（首次对账即误读，只读无副作用）。发布物与断点细节见任务卡第 3、5 节「断点与交接物」。


> **2026-09-16 LQ-34 通用钱包打通（测试实例已灰度通过；生产未发布、迁移未执行）**。老板口径：「**兰琪的用户只在兰琪里充值；在兰琪充的积分，可以同时在思潼 AI 里用其他智能体**——钱包是通用的」；多人门店「**不管谁操作，都扣租户老板（owner）的钱包，流水里记『谁操作的、扣的是老板的钱』**」；迁移选 **Phase 2（不做双轨过渡，只留一本账）**；不显示人民币。**交付内容**：① 新增 `apps/api/src/services/lanqi-wallet.ts`，把「谁付钱」收口成三个函数 `precheckLanqiWallet` / `chargeLanqiWallet` / `refundLanqiWallet`（+ `readLanqiWalletBalance`）——按 `tenantId` 解析 owner，解析不到就 **fail closed**（`lanqi_wallet_owner_missing`，不扣费也不放行）；幂等键统一 `lanqi:{tenantId}:{requestId}`；扣费走 `consumeWalletCredits`（paid→bonus、Serializable、同键幂等）；**退款按原扣费流水分桶退回原桶**；操作人写进流水 `source`（`lanqi:operator=<userId>`），付费人与操作人分开可审计。② **5 个扣费点切到 owner 钱包**：文案十件套（`88f6f82`）、图片 / 视频确认（`f1b8057`）、爆款复刻出片 + 出片许可预算（`16d4a17`）、视频执行 seedance（`d3bb128`）——③④⑤ 同属一套「预留-结算」模型，**一次切完**（拆开切会留半套逻辑）；页面余额同步改读 owner 钱包，与「我的 · 充值」同一本账。**③ 顺带纠了一条会烧到别的产品的迁移口径**：初版迁移脚本按「全库 `CreditAccount.balance > 0`」取数，生产实测会搬 **201 个账户 / 2,001,160,483 积分**，其中 **199 个 / 2,001,159,975 属于思潼 AI（`founder-ip,takeaway` 190 个）与美业（4 个）**——属跨产品越权改账；已收窄为「只迁持 `lanqi` 权益 / 有 `LanqiStoreProfile` 的租户」（`9c61771`），生产迁移候选降到 **2 个账户 / 508 积分**（QA-20260916-011）。**验证**：`pnpm.cmd qa:fast` **exit 0**；`pnpm.cmd qa:regression` **exit 0**（首两条 `LANQI_WALLET_REFUND_DRILL_PASS passed:25`、`LANQI_WALLET_MIGRATION_SCOPE_DRILL_PASS passed:20`）；**测试实例真实灰度演练 38 断言 / 0 失败**（新增 `scripts/lanqi-wallet-instance-drill.mts`，真实 HTTP + 真实 prisma + 真实模型：充值前 402 且不扣费 → `applyRecharge` 充 100 后同一接口 200 / 扣 40 / 流水记操作人 → 跨租户 404 且 B 店钱包不动 → 无 owner 409 fail closed → 跨桶扣费原桶退回 / 重复退款幂等 / 已退款 requestId 重放被拒 → 全程旧账 `creditAccount` / `creditTransaction` / `creditReservation` 零写入；跑完自动清理，服务器残留核查全 0）；测试实例全量发布发布 id `lq34-wallet-full-20260916-1335`（1576 文件 / 9,937,236 B / sha256 `e8cd44eabd8adf63ec63431f21d7d778cb2f105590c181c8cd72e71e1bab4925`，`DEPLOY_OK` + `health/ready 200` + `No pending migrations`，回滚备份 `/opt/baolu-backups/lq34-wallet-full-20260916-1335-before-baolu-os-v2-test/` 200M）。**边界与未做（如实说）**：**生产未发布、历史额度迁移未执行**——本轮只出 dry-run，`--apply` 等老板确认；⑥⑦（经营问答的 `credit-reservations` 与美业单品预留 / 结算线）本轮**不动**，它们被美业单品复用，要为它单独立卡 + 跨产品回归；生产候选里 `cmt6wlw7w0517pou3005wvk8o` 那条历史上有 `reservation:beauty-industry:*` 流水，是否随迁需拍板。**待老板拍板**：① 迁移 dry-run 报告确认（生产 **2 个账户 / 508 积分**，其余 199 个正确排除）；② 生产发布窗口。详见任务卡 `docs/agents/lanqi-beauty/tasks/LQ-34-兰琪通用钱包打通.md` 与 `docs/BUG_REGRESSIONS.md` QA-20260916-011。

> **2026-09-16 LQ-33 上线（测试实例 + 生产）：公域获客新增「美业文案十件套」卡（独立计费）**。用户 2026-09-15 口径：「**新增一张卡**（清晰、独立计费）」。**交付内容**：① 公域获客枢纽页第 1 张卡「美业文案十件套」→ `/lanqi/acquire/copy-kit`；门店说清「主推什么项目 / 想让谁看到 / 顾客最怕什么」，一次拿到「内容十件套 V5」全套正文，可**复制整份 / 导出 Markdown**；关键信息缺失时先反问，不编造门店事实。② **合同唯一出处**：新增 `apps/api/src/products/beauty-industry/copy-ten-contract.ts`（十节常量 + 提示词 + 结构校验），货架 `marketplace.ts` 改为引用它并删除本地副本——货架「文案智能体」与兰琪这张卡**共用同一份合同**，不存在第二份提示词。③ 兰琪侧新增 `products/lanqi/copy-kit-service.ts`（只加「怎么问、缺信息怎么办」的提问壳）与 `POST /lanqi/acquire/copy-kit`：门店可见性校验 + 租户隔离、余额不足先 402（**不调模型**）、模型失败 / 信息不足 / 结构不合格**一律不扣积分**、扣分用条件更新（并发不为负）并写 `creditTransaction`、结果按租户哈希目录隔离、同 `requestKey` 同输入复用结果（重复点不重复扣）、同键换输入 409。④ 计费：**独立口径默认 40 积分 / 次**（与货架同合同同档价），`LANQI_COPY_KIT_CREDITS` 可覆盖（调价不发版）；`LANQI_COPY_KIT_MAX_TOKENS` 可调输出额度（默认 16384）。**验证**：`lanqi:copy-kit-smoke` **28/0**、`lanqi:acquire-ui-contract-smoke` **131/0**、**真实模型 Eval `lanqi:copy-kit-live-eval` 同一高风险样例 3 次 15/0**（三次 `finishReason=stop`、正文 3.1k–3.3k 字、十节齐全、无样板门店泄漏）、`pnpm.cmd qa:fast` exit 0、测试实例真实浏览器 **46 项 / 失败 0**（含移动端 390 无横向溢出）；发布 id 测试 `20260916-lq33-copy-kit-test1` / 生产 **`20260916-lq33-copy-kit-prod1`**，两侧 `DEPLOY_OK` + `verify-deploy.sh` **VERIFY_OK**，生产匿名 `POST /os-v2/api/lanqi/acquire/copy-kit` **401**、线上 chunk 含页面钩子与枢纽卡。**本轮修掉两个真问题**：① 真实生成偶发失败（推理 token 吃满 8192 输出额度被截断 / 口播字数不足 / 合规备注里写了被禁词字面被结构校验打回）→ 输出额度提到 16384 + 提问壳补齐四条硬要求，3 次 Eval 全绿（QA-20260916-002）；② Windows 上浏览器探活用 `--version` 被 Chrome 当普通启动而假失败「未找到可用的 Chromium/Chrome」→ 改 headless `--dump-dom` 探活（QA-20260916-001）。**边界**：生产页面级验收未跑（需真人微信登录，既有边界）；真实付费端到端未跑（测试租户积分不足）；本卡只出文案与脚本，不做图片 / 视频 / TTS 联动。**待老板拍板**：本卡单价（现默认 40 积分/次）；**积分账户口径**——兰琪扣的是租户 `creditAccount`，`/recharge` 充的是用户 `wallet`，代码里没有同步链路，若充值后仍提示「积分不足」需单独立项。详见任务卡 `tasks/LQ-33-公域获客文案十件套新卡.md`、`docs/BUG_REGRESSIONS.md` QA-20260916-001/002 与 `docs/CURRENT_DEPLOYMENT_STATUS.md` 顶部条目。

> **2026-09-15 充值入口接通（已上测试实例 + 生产，发布 `20260915-lq33b-recharge-entry`）**：老板问「兰琪智能体在哪里充值」→ 查证发现兰琪顶栏「我的」仍指向 `/my-ai`，而该地址同日已随平台发布下线并跳智能体货架，**门店在兰琪里点不到充值**。最小修复：顶栏改直达钱包页 `/recharge`、文案改「我的 · 充值」；「一键成片」两条积分不足提示改为「请点右上角「我的 · 充值」」。回归：`lanqi:brand-nav-contract-smoke` **49/0**（新增 3 条，含禁止 `/my-ai`）、`lanqi:acquire-ui-contract-smoke` **102/0**、测试实例真实浏览器 **43/0**（新增顶栏 href 断言）、生产主包引用 chunk 内 `/recharge`×1 / `/my-ai`×0、`GET /os-v2/recharge` **200**、`verify-deploy.sh` **VERIFY_OK**。**门店充值路径**：登录兰琪工作台 → 右上角「我的 · 充值」→ 钱包页（余额 / 套餐 / 订单），或直达 `https://api.lcppch.top/os-v2/recharge`。详见 `docs/BUG_REGRESSIONS.md` **QA-20260915-006** 与 `docs/CURRENT_DEPLOYMENT_STATUS.md` 顶部条目。

> **2026-09-15 LQ-32 上线（测试实例 + 生产）：一键成片音频接通——上传音轨（或带声音的视频）→ 按分镜拼成一条整片并混音**。用户本轮口径两步：①「一键成片里的音频上传暂未接通（本期成片无声）——让用户上传一段音频就可以了或者支持上传视频抽取音频也可以啊」；②「A 新增一张卡（清晰、独立计费）「音频 + 文案一起做」」。**先说清技术事实**：视频模型 `wan2.6-i2v-flash` **只出无声画面**（`parameters.audio=false`、不下发 `audio_url`），且改造前「一键成片」只按镜交付、**没有拼接**，所以音频卡即便能上传也没有落脚点——声音只能由平台本地混流补上。**交付内容**：① 音频卡由占位改为**真实上传**（`audio/*,video/*`，≤20MB；上传视频时由平台 ffmpeg 抽其声音当音轨），多段必须显式点「设为本片音轨」，并新增**音轨使用权授权**（未勾选不合成）；② 新增 `apps/api/src/services/lanqi-media-compose.ts`：按分镜顺序拼接（`scale+pad+setsar+fps=30` 统一画布 + `concat`）、混音（`-stream_loop -1` 循环补齐 + `-shortest` 以画面长度为准 + `-c:a aac`）、输出校验（`ftyp` 魔数 + 体积上限）、`requestKey` 幂等、**租户隔离**（镜次按 `tenantId` 查、音轨必须是本租户 `UploadedFile` 且落在 `UPLOAD_DIR`、成片路径由租户派生）；③ 新增 `POST /lanqi/media/compose` + 成片播放 / 下载两条 GET（均先解析登录态，匿名 401）；④ 页面上「输出规格 / 成片区」显示「🔊 带音轨成片 / 🎧 抽音轨成片」与「🧩 合成成片（拼接 + 混音）」，未出片时禁用并点名「还差 N 镜没出片」；旧文案「本期成片无声 / 音频上传暂未接通」清零。**计费**：合片与混音是本机 ffmpeg 计算，**不额外扣积分**（页面已写明）；逐镜出片与积分单价**未动**。**验证**：新增离线回归 `pnpm.cmd lanqi:media-compose-smoke` **22/0**（无音轨拼接 / 长音轨 / 短音轨循环 / 视频抽音 / **无声视频当音轨必须报 `audio_track_missing`** / 跨租户 404 / 未出片 409 / 未授权 400 / 超大音轨 / 幂等 / 成片跨租户读不到）；`lanqi:acquire-ui-contract-smoke` 新增 22 条断言 → **101/0**；测试实例真实浏览器 `lanqi:acquire-instance-acceptance` **42 项 / 失败 0**；`pnpm.cmd qa:fast` **exit 0**；发布 id 测试 `20260915-lq32b-audio-compose-test1` / 生产 **`-prod1`**，两侧 `DEPLOY_OK` + `health/ready 200` + `verify-deploy.sh VERIFY_OK`，生产匿名探针 `POST|GET /os-v2/api/lanqi/media/compose` 均 **401**，线上 chunk 含 `data-lq-vd-compose` / `上传音频` / `合成成片`、旧文案 0。**顺带修掉一个门禁假失败**：验收脚本 `findChrome()` 只看文件是否存在，选中了启动即退出的 Playwright Chromium，导致每次验收都以「DevTools 端口未就绪」假失败（QA-20260915-005，已改为 `--version` 探活）。**边界（如实说）**：**真实付费素材端到端尚未跑过**（测试租户 0 积分，逐镜真实出片要积分）——合成链路用真实 ffmpeg + 合成素材离线验证；单条成片 ≤12 镜、音轨 ≤20MB；不做 TTS 自动配音 / 多音轨混音 / 字幕烧制；`VIDEO_RENDERING_READY=false`（门店素材成片 / AI 剪辑）继续 fail-closed。**下一步**：LQ-33「美业文案十件套」新增一张卡 + 独立计费（任务卡已建，需动平台热点文件 `apps/api/src/routes/marketplace.ts` 抽共享合同，须与平台线串行）。详见 `docs/CURRENT_DEPLOYMENT_STATUS.md` 顶部条目、`docs/BUG_REGRESSIONS.md` QA-20260915-005 与任务卡 `tasks/LQ-32-一键成片音频接通.md`。

> **2026-09-14 LQ-30 上线（测试实例 + 生产）：爆款复刻改成「只上传原片」，出片链路第一次真的能跑通**。用户本轮口径三步演进——先「按推荐做」（出片许可自动签发 + 在线勾选即算授权声明），再「先取消抖音链接的爆款复刻，只支持上传视频」，最后「不用给杨萋萋账号补权益、不用合成素材，我自己用真素材测验」。**交付内容**：① 参考素材只留「上传原片」——抖音链接页签 / 输入框 / 「登记参考来源」按钮 / 链接解析代码整体撤退（生产产物实测 `参考抖音链接`=0、`lq-vd-ref-link`=0、`登记参考来源`=0，`上传原片`=5）；② 出片链路的产品权益清单抽到 `apps/api/src/services/video-replication-entitlement.ts`（`beauty-industry` + `lanqi`），路由准入 / 素材授权 `scope()` / 许可 `currentAccess()` 三处硬编码 `beauty-industry` 的历史问题一次收口——此前兰琪租户一点「先报价，再出片」必 403；③ 新增 env `VIDEO_REPLICATION_PERMIT_MODE`（默认 `operator` 与改动前完全一致；生产与测试实例置 `auto`）——按同一套预算上限自动签一条绑定本次请求的单批许可，只能 claim 一次、submit 一次，轮询/下载/暂存各自限额，扣分与幂等不变，已 claim 或已撤销的许可不重签；④ 页面报价前自动上传「在线勾选声明」文本作为授权依据并登记两份素材授权（幂等），不再要求门店上传 PDF；⑤ 报价缺口翻人话（`insufficient_credits` 等），确认仍走既有 402。**验证**：新增 `pnpm.cmd lanqi:video-replication-access-smoke` **19/0**（红→绿；覆盖只有 lanqi 权益→报价 200、无权益仍 403、`operator` 模式仍 422、重复报价不重复签许可、积分不足提前点名、美业租户回归）；`beauty-industry:video-execution-smoke` 三轮 PASS、`video-material-authorization-smoke` BY46_PASS、`video-foundation-smoke` BY45_FOUNDATION_PASS、`lanqi:acquire-ui-contract-smoke` **79/0**（含四条「不再有贴链接入口」反向断言）；测试实例真实浏览器 **37/0**；发布 id 测试 `20260914-lq30b-replicate-live-test1` / 生产 `20260914-lq30b-replicate-live-prod1`，两侧 `DEPLOY_OK` + health/ready 200，生产 `verify-deploy.sh` **VERIFY_OK**。**边界与未做（如实说）**：出片仍必须有原片——抖音/视频号不开放站内视频文件下载（百炼 `wan2.2-animate-mix` 要的是**视频文件公网直链**，不是平台分享链接；我们实测 4 条路径全空壳），要真做到「贴抖音口令出片」必须另接第三方解析/数据服务（采购决定）或抖音开放平台授权；单条成本上限仍 ¥10 → 只能出 ≤16 秒的片；**未给任何租户补权益、未跑真实付费样片**（用户自行用真素材测验）；老板租户当前 0 积分，出片前需充值。详见 `docs/BUG_REGRESSIONS.md` QA-20260914-005 与任务卡 `tasks/LQ-30-爆款复刻真实可出片.md`。**注意**：并行任务提交 `cd37a6b` 一度把工作树页面覆盖回贴链接旧版，已由 `881d7be` 重新落回只上传原片版。

> **2026-09-14 LQ-29 爆款复刻参考素材三条现场缺陷修复（已上线：测试实例 + 生产；出片权益口径仍待拍板）**：用户 2026-09-14 在本任务反馈三条现场缺陷（附 3 张截图）——① 粘贴抖音「分享 → 复制链接」的口令被判成「这不像一条完整链接」；② 已上传的照片 / 视频无法删除、无法替换；③「先报价，再出片」点不了。**三条均已复现 → 定位 → 先红后绿修复（只动前端，未改后端路由 / 鉴权 / 计费）**：`parseReferenceLink()` 重写为「先从任意粘贴文本抽出第一条链接再校验」（`http` 升级 `https`、裁掉链接后粘连的中文与标点、无协议头短链 `v.douyin.com/xxx` 也认），整段确实没有链接时明说「这段文字里没有链接」并给「分享 → 复制链接」步骤，非抖音链接报出**实际域名**，旧提示「这不像一条完整链接」彻底删除；上传区新增「🔄 更换原视频 / 🔄 更换照片」与「🗑 删除这条原片 / 删除这张照片」（钩子 `data-lq-vd-remove`），删除时**换新幂等键**并清掉上一次报价 / 任务 / 成片，重传即全新一次请求；出片主按钮改为显式状态机 `missing / need_quote / ready / blocked`（钩子 `data-lq-vd-primary`）——素材没齐就点名「还差：…」，素材齐了但没报价时**可点**、点它就是先报价，报价可确认后变「✅ 确认并出片（按报价扣 N 积分）」，**未确认前不建任务、不扣积分**；`readResponse()` 把错误码挂到 `error.code`，新增 `replicationFailureNotice()` 把 403 说成人话（「当前账号还没有开通这项出片能力，与素材、授权是否填对无关」）。**回归（先红后绿）**：新增 `scripts/lanqi-acquire-reference-link-smoke.mjs`（11 条；指向修复前源码 9 条红 → **11 passed / 0 failed**）、`lanqi:acquire-ui-contract-smoke` 新增 16 条源码结构断言（未改页面时全红 → **82 / 0**）、`lanqi:acquire-instance-acceptance` 新增 LQ-29 浏览器验收段（共 **42 项**，并修掉探针自身两处缺陷：请求时间线漏统计 `/viral-video-replication/`、照片未上传文案写死）、`pnpm.cmd qa:fast` exit 0。**发布**：`release-20260914-lq29-acquire-fixes-v2.tar.gz`（9,623,158 B / 1512 文件，sha256 `bf1a1bcbad7760c138e2afa027f5704fdd0b01091b67dd063e6f9ac10bd5f29c`，本地与服务器 `/opt/releases/` 一致），发布 id `20260914-lq29-acquire-fixes-test2` / **`-prod1`**，两侧 `DEPLOY_OK` + `health=200` / `ready=200` + `50 migrations found / No pending migrations`，`verify-deploy.sh` 两侧 **VERIFY_OK**。**生产只读取证**：`index.html` → `assets/index-DNYd7rb4.js` → `assets/LanqiAcquireVideoPage-BwFfTi2O.js`，直接下载该线上 chunk（HTTP 200 / 67,224 B）实测 `data-lq-vd-primary` / `data-lq-vd-remove` / `data-lq-vd-primary-hint` / `还没有开通这项出片能力` / `这段文字里没有链接` / `更换原视频` / `更换照片` / `删除这条原片` 各命中 1，旧串 `这不像一条完整链接` **0**；`POST /os-v2/api/viral-video-replication/quote` 未登录 **401 `unauthorized`**。**页面级验收**：测试实例真实浏览器（桌面 1440 + 移动 390，CDP `DOM.setFileInputFiles` 真实上传）**42 项 / 失败 2 项**，两项失败**均为权益门禁的预期结果**（免登录新建租户只带 `lanqi` 权益 →「点主按钮真的走报价」「无接口 4xx/5xx」必然被后端 403 挡下），**不是 UI 缺陷**；其余 40 项全 PASS（含三条缺陷红绿复验、分享口令识别、删除 / 更换、主按钮状态机、390 无横向溢出、console / page 0 错误）。**生产页面级浏览器验收未跑**（生产 `/lanqi/acquire*` 需真人微信扫码登录，自动化停在 `/os-v2/login`，同 LQ-22 / LQ-28 既有边界）。回滚＝`/opt/baolu-backups/20260914-lq29-acquire-fixes-prod1-before-baolu-os-v2/`（190M）+ `systemctl restart baolu-os-v2`。**⚠️ 仍未闭环（需老板拍板，属权限 / 计费口径变更，Agent 未擅自执行）**：前端放开按钮后仍拿不到报价——`apps/api/src/routes/viral-video-replication.ts` 的 `context()` 硬编码只认 `productCode:"beauty-industry"` 的 active 权益，而兰琪租户只带 `lanqi`（生产只读实测：active 权益 `takeaway=190 / founder-ip=190 / lanqi=9 / beauty-industry=7`，**9 个 lanqi 租户里只有 2 个同时持 `beauty-industry`**，且都是 2026-08-24 的历史租户）→ 老板一点就走 403。二选一：① 给在用的兰琪租户补 `beauty-industry` 权益（只改数据）；② 放宽 `/viral-video-replication/*` 接受 `lanqi` 权益（改公共路由鉴权，风险更高，需完整回归）。另有事实待确认：在用的兰琪租户与历史上传素材的租户可能不是同一个（老板可能换过微信号 / 租户）。详见 `docs/BUG_REGRESSIONS.md` QA-20260914-004 与任务卡 `tasks/LQ-29-爆款复刻参考素材三条缺陷.md`。

> **2026-09-14 LQ-28 发布（已上线：测试实例 + 生产）**：`release-20260914-lq28-self-material-v2.tar.gz`（9,515,966 B / 1493 文件，sha256 `170d6fbf4d645c4dbd3998175fbc7683d4975e23fe9beae396e33ed9fdd1712a`），发布 id `20260914-lq28-self-material-test2` / **`-prod1`**，两侧 `DEPLOY_OK` + `health=200` / `ready=200`，删除清单 4 条 `viral-search*` 实测全部 `removed`，`prisma migrate deploy` = `No pending migrations`。**生产只读取证**：`POST /lanqi/acquire/video/viral-search` 发布前 **401**（路由存在、需登录）→ 发布后 **404 `Route … not found`**；`/opt/baolu-os-v2` 下 4 个 `viral-search*` 源文件已删除；线上入口产物 `assets/LanqiAcquireVideoPage-Cp5WhOXM.js` 内 `平台筛选`/`行业领域`/`lq-vd-kw`/`viral-search` 命中 **0**，`搜爆款` 仅剩 2 处说明文案（「平台不再替你去搜爆款」）。页面级验收：测试实例真实浏览器 **33 项 / 失败 0**（桌面 1440 + 移动 390）。**生产页面级浏览器验收未跑**（生产 `/lanqi/acquire*` 需真人微信扫码，自动化停在 `/os-v2/login`，同 LQ-22 既有边界）。回滚＝`/opt/baolu-backups/20260914-lq28-self-material-prod1-before-baolu-os-v2/` + `systemctl restart baolu-os-v2`。开发口径与取证明细见下方同日的 LQ-28 段落。
...
> **2026-09-14 LQ-28 爆款复刻取消「搜索爆款」，改为门店自备素材（已下线 LQ-25）**：用户 2026-09-14 口径「**取消爆款复刻里面的搜索爆款功能，让用户自己添加链接或者上传视频文件**」。触发原因是线上真实结果不可用（抓到的不是爆款、要皮肤管理给沐足、还混入图文）；在生产机用同一把凭据跑了 6 条只读探针后确认这是**能力天花板而非排序问题**——检索接口只回 `icon/site_name/index/title/url`，**没有点赞/播放字段**（证明不了爆款）；`site:channels.weixin.qq.com` **0 条**（视频号侧无可检索站内视频页）；抖音那组 10 条里 **0 条 `/video/`**、3 条图文、3 条账号主页，并含用户点名的「楠枫沐足」；抖音视频页 / `iesdouyin iteminfo` / `aweme detail` / 分享页**四条路径全部为空壳或 0 字节**（服务端拿不到视频文件）。**处置：LQ-25 整条链路下线**（页面入口、`/lanqi/acquire/video/viral-search` 路由、`viral-search-rules.ts` / `viral-search-service.ts`、`LANQI_VIRAL_SEARCH_*` 配置、`lanqi:viral-search-contract-smoke`），页面改为三段式自备素材：① 参考素材（「🔗 参考抖音链接」只登记来源、不发请求不出片并说明平台不提供视频下载；「🎬 上传参考视频」收 `MP4/MOV ≤200MB` 并回显真实时长）→ ② 人物形象（换脸 / 换人 + 肖像图）→ ③ 素材与肖像授权 → 报价 → 确认 → 轮询 → 播放下载（沿用 LQ-27 链路，不动计费与幂等）。回归：`lanqi:acquire-ui-contract-smoke` **65/0**（整段换为 LQ-28 反向+正向断言）、测试实例 `lanqi:acquire-instance-acceptance` 真实浏览器、`pnpm.cmd qa:fast` exit 0。详见 `docs/BUG_REGRESSIONS.md` QA-20260914-003 与任务卡 `tasks/LQ-28-爆款复刻取消检索改自备素材.md`。
...
> **2026-09-12 LQ-26 视频获客一期收口（两页：爆款复刻单模式 + 一键成片 6 步，已上测试实例与生产）**：按用户指定入口 `CODEX-START-HERE.md` → `CODEX-开发总纲-20260909.md` → **`2026-09-12-视频获客一期最终范围-Codex交接.md`**（本期以此为准，本地工作副本 `beauty-xhs-prototype-20260827\video.html` / `video-copy.html` 为活规范）执行：视频获客由「一页四页签」改为**两个独立路由**——`/lanqi/acquire/video` = 🔥爆款复刻单模式（去掉页签，无门店素材成片 / AI 剪辑入口，两段代码保留在文件内但不再渲染），`/lanqi/acquire/video-copy` = **一键成片**（原「文案转片」改名，**删掉「手动贴文案」整段 UI**，6 步：说需求 → AI 生成文案 → AI 分镜脚本 → 传素材卡 → 积分预算 → 成片）；旧深链 `?mode=script` 一跳转到一键成片。新增后端 `POST /lanqi/acquire/video/copy-candidates`（`apps/api/src/products/beauty-industry/video-copy-service.ts`）：需求门禁（必填 / ≤120 字 / 违规引导词拦截）、四种风格齐全校验、**固定出 3 版且第 1 版 = 用户选的风格**、「换一批」只轮换后两版、字数与预估口播时长**由后端计算**、需求里没出现过的数字进 `warnings` 提示核对（不编造门店事实）；文案由真实大模型生成，前端不拼模板，失败一律失败关闭（503 未配置 / 502 结构失败，不返回演示文案）。枢纽页改为两张视频卡（爆款复刻 / 一键成片）。回归：新增 `scripts/lanqi-video-copy-contract-smoke.ts`（**32/0**，已进 `qa:fast`）；`pnpm.cmd qa:fast` **PASS**；测试实例真实浏览器验收 `pnpm.cmd lanqi:acquire-instance-acceptance` **20 项 0 失败**（爆款复刻单模式 tabs=0、真实检索 8 条全在平台域内、一键成片首屏 6 步且无贴文案入口、真实大模型出 3 版候选、「用这版」直达「第 3 步 / 6」分镜、console/page 0 错误、390×844 无横向溢出），并顺带纠正一条**既有失效断言**（LQ-23 起视频页在「收口」下刻意可直达，旧脚本仍按「开发中占位」判定）。发布包 `release-20260912-lq26-video-two-pages-full.tar.gz`（sha256 `e86935416c31727041be75de806d49cdafb910a7ce75f29d24f662b566d8ee87`），发布 id `20260912-lq26-video-two-pages-test1` / `-prod1`。**已知边界**：爆款复刻真实出片仍 fail-closed（`VIDEO_RENDERING_READY` 未开 + 生产 `ALIYUN_VIDEO_REPLICATION_MAX_COST_FEN` 未配，属演示 4 的 OSS/STS 与预算闸那条线）；门店素材成片 / AI 剪辑按口径不交付。详见任务卡 `tasks/LQ-26-视频获客一期两页收口.md`。

> **2026-09-12 LQ-25 爆款复刻真实检索源接通（抖音 + 视频号，只在兰琪用）**：用户 2026-09-12 拍板「爆款复刻的检索源 = **抖音和视频号两个平台**」并指示「**开闸跑**」，同时明确「**爆款复刻暂时只在兰琪去用**」。本轮把兰琪视频页「爆款复刻」页签从硬编码 `REPLICATE_SEARCH_READY=false` 换成真实检索：新增规则层 `apps/api/src/products/lanqi/viral-search-rules.ts`（只认抖音 `/video`·`/note` 与微信生态站内页，账号主页 / 搜索页 / 开放平台文档 / 第三方站点 / 短链 / `http` 明文一律丢弃；同 canonical 去重；**条目不带播放量点赞字段，不做热度推算**）、服务层 `viral-search-service.ts`（可替换 Provider 合同 + 真实联网检索 Provider，**只读 `search_info.search_results` 来源页清单、不用模型结论**；未配置→`viral_search_unavailable` 失败关闭；上游全失败→502；上游全是域外站点→空数组 + 明确文案）、兰琪专属路由 `POST /lanqi/acquire/video/viral-search`（同一套门店隔离与 RBAC，单店角色检索不到别家门店；**不扣费、不写流水、不落库**，一期不做积分）。页面新增检索结果区（平台 / 真实类型徽标 + 「打开原页面」+「选它复刻」），无有效条目时照实说明。环境变量默认 `LANQI_VIRAL_SEARCH_DRIVER=disabled`（漏配即关闭），生产 / 测试实例改为 `aliyun_web_search` 开闸。**实测**（生产同一把凭据、跑真实服务代码）：关键词「皮肤管理门店获客」→ 抖音 **3 条**站内页、视频号侧 **6 条**微信生态页、全部平台 **6 条**合并结果，URL 全部落在 `www.douyin.com/{video|note}` 与 `mp.weixin.qq.com/s?...`，无第三方站点、无编造字段。回归：新增 `scripts/lanqi-viral-search-contract-smoke.ts`（**55/0**，已进 `qa:fast` 与 `lanqi:acquire-smoke`）、`lanqi-acquire-ui-contract-smoke` 新增 7 条视频页契约、`lanqi-acquire-instance-acceptance` 爆款复刻断言改为「必须真发请求 + 条目只能平台域内 + 无有效条目必须明确说明」。**已知边界**：视频号站内视频页无公开索引（`site:channels.weixin.qq.com` 实测 0 条），「视频号」档当前给的是微信生态公开图文页并按真实类型标注，不冒充视频号视频；要拿真正的视频号视频条目只能走第三方数据服务或微信侧官方能力（需用户另行决定与授权）。详见任务卡 `tasks/LQ-25-爆款复刻真实检索源接通.md` 与 `docs/BUG_REGRESSIONS.md` QA-20260912-017。
>
> **⚠️ 2026-09-14 已由 LQ-28 整体下线**：本段所述检索能力（页面入口 / `POST /lanqi/acquire/video/viral-search` 路由 / 规则层 / 服务层 / `LANQI_VIRAL_SEARCH_*` 配置 / 专项冒烟）**已全部删除**，爆款复刻改为门店自备素材。本段保留为历史留痕，**不代表当前产品行为**。见 QA-20260914-003 与任务卡 `tasks/LQ-28-爆款复刻取消检索改自备素材.md`。

> **2026-09-12 追加（爆款复刻检索源口径：先走 A）**：用户拍板**本期不接第三方付费数据源** —— 爆款复刻的「视频号」档继续用公开网页检索给的微信生态公开图文，并按真实类型标注，不冒充视频号视频。**B（第三方数据服务）的触发点**：等**爆款复刻真正出片跑通**（需用户侧 OSS/STS 凭据 + 生产付费上限）之后，由 Codex 核对这条链路的实际价值，再回来决定要不要为「视频号视频条目 + 播放点赞数据」付费。**在此之前不得擅自开户、采购或引入任何外部付费数据源。**

> **2026-09-13 私域营销内测验收：WorkBuddy 报告 15 项 14 通过，唯一不通过项已修复并上两环境（QA-20260913-005）**：WorkBuddy 交付《兰琪私域营销内测验收报告》（测试实例）。**复核结论：14 项通过属实；唯一不通过 #9「失败 / 超时 / 断网」成立且是真 Bug** —— 断网时页面把 `fetch` 的 `TypeError: Failed to fetch` 原文直接渲染，既非中文人话也没有重试入口（输入保留、刷新恢复、复制/重新生成、信息不足分支、微信群主题留空、多端同步反馈、移动端 390、模型中立、合规文案、租户隔离等其余项均通过）。**已最小修复**：新增 `apps/web/src/lib/humanize-error.ts`（网络/超时 → 中文人话 + 「点『🔄 重新生成』再试一次，刚才填的内容不会丢」；未知英文错误中文兜底；已有中文业务提示原样保留），朋友圈页与微信群页 `catch` 统一改用它，报错态加 `role="alert"` + 「🔄 重新生成」按钮（`data-lanqi-retry`）；朋友圈 AI 配图失败同口径。**先红后绿**：新增 `scripts/lanqi-moments-error-copy-smoke.mjs`（修复前 `EXIT=1` → 修复后 **11/0**，已挂 `qa:lanqi-foundation`）；`lanqi:test-instance-acceptance` **12/0**；web typecheck 通过；两环境产物含新文案且不再出现该英文串。发布包 `release-20260913-qa1301-moments-error-full.tar.gz`，发布 id `20260913-qa1301-moments-error-test1` / `-prod1`，两环境 `DEPLOY_OK`。**待复跑**：WorkBuddy acc-run.js 第 9 项确认中文提示 + 重试入口；#15 租户隔离建议在生产双真人账号下复验。
...
- 服务端行为（`apps/api/src/routes/auth.ts` 的 `resolveWechatLogin`）：无 membership → 200 `{needsTenant:true, onboardingToken}`（去补资料开通）；**已开通别的产品但没有本产品租户 → 同样走 `needsTenant`**（用户 2026-09-12 拍板「一个账号可以使用全平台」，旧的 403 `product_membership_required` 已移除）；受控产品（兰琪）的开通闸门是**产品邀请码**，补资料那条 `onboarding/create-workspace` 仍强制校验，无码即 403 `invite_code_required`。回归锁：`scripts/product-login-entry-smoke.mjs`（QA-20260912-014）。
...
- 「公域获客页尽可能开发完」：LQ-19 已在 2026-09-11 上生产并专项验收（见下节）。四模式里 `copywriter` / `live` / `methods` 可用；`video` 的 `replicate` **自 LQ-28（2026-09-14）起改为门店自备素材**（贴抖音参考链接登记来源 / 上传参考视频原片 + 人物形象，见本页顶部 LQ-28 段），不再有「搜爆款」入口；`assets` / `clip`（出片服务未开通，`VIDEO_RENDERING_READY=false`）为**设计内 fail-closed**，页面明确提示缺口、不假装成功——属「能跑 / 待外部条件」，非缺陷。测试实例实测 **30/0**。
...
> **2026-09-12 口径拍板（多版本切换）**：用户明确「**不要多版本切换，如果不满意，支持用户重新生成就可以了**」。现状即满足：结果卡片与「信息不足」分支都有「🔄 重新生成」，且后端每次调用都真实重跑一次生成（幂等记账不返回旧结果），**无需新增代码**。此条关闭，不再作为待决策项。详见任务卡 `tasks/LQ-24-私域营销信息不足分支补齐重新生成.md`。
> **2026-09-12 爆款复刻暂存链路接通（OSS + STS + 24h 保留）**：用户拍板「给用户保存 24 小时，24 小时之后在服务器里删掉」。本轮完成：① 阿里云 **OSS 专用桶 `lanqi-video-staging`**（华北2北京 / 标准存储 / 本地冗余 / 私有 / 防误删关闭 / 阻止公共访问开通，控制台建）；② **1 天生命周期规则**（前缀 `beauty-industry/video-staging/v1/lanqi-prod/`、Enabled、无 Tag/Filter）——**驱动器视角实测读到** `[{prefix,status:Enabled,expiration:{days:"1"}}]`，通过 `Number(days)===1` 校验；③ **RAM 角色 `baolu-video-staging` + 专用用户 `baolu-runtime`**（只允许 AssumeRole），STS 三元组 12 小时有效、已写入生产 env，**STS 只写不打印**（只留 SHA 指纹）；④ **自动续期** `bs-video-sts-refresh.timer`（每 10 小时，到期时间变化才重启）；⑤ **24h 服务器侧清理** `bs-video-retention.timer`（每小时删 `uploads/.beauty-video-results` 与 `uploads/lanqi-media/staging` 中 24 小时前的文件，OSS 侧由生命周期兜底）。运维脚本入库：`scripts/bs-video-sts-refresh.mjs` / `bs-video-sts-refresh-run.sh` / `bs-video-retention.sh`。**仍未开**：正式路由预算 `maxCostFen` 仍为 0（付费执行关闭），`BEAUTY_VIDEO_EXECUTION_MODE` 未开 → 不会误出片、不会花钱；下一轮接预算（¥10 上限）后跑真实样片。踩坑记录：`gosspublic` 上 `ossutil 1.7.19` 是 404（返回 XML 错误页），**1.7.16 才有**；ali-oss 的 `getBucketLifecycle` 必须传桶名，否则读回 null。
