...
- 分层路由启动条件（2026-09-17 定，Codex 与老板确认；本段只追加，不改上文）：本候选通道**不设启动日期**，触发点是「第一张天然低风险的批量调用卡」。四条同时满足才立卡 —— ① 业务：出现低风险批量调用（同一次请求内重复几十上百次、只做分类/打标/抽取/格式整理、客户不直接把它当交付物）；② 工程：那张卡新增调用点时必须同时把 `createLanqiTaskLlmProvider(taskClass)` 接进生产路径（截至 2026-09-17 它在 `apps/api/src` 内零生产调用点，兰琪内容/图片工作室走 `assertLanqiProfessionalTextModel()`，模型不是 `deepseek-v4-pro` 直接抛 `lanqi_pro_model_required`，所以单开两个 env 不改变任何生产行为）；③ 收益：按真实用量估「该能力占产品侧文本调用 ≥30%」且「预计月省 ≥ ¥300」（flash 单价约为 pro 的一半，省额 ≈ 低风险调用占比 × 50%，属线性小钱，不是数量级；50 倍那个差是同一模型内「缓存命中 vs 未命中」）；④ 前置：兰琪无未解决 P0/P1，且 LQ-34 老板门店实跑已通过 —— 钱包刚切换的窗口不做模型降级，否则出问题分不清是计费的毛病还是模型的毛病。触发后由 Codex 出 Eval 基线方案：Champion = `deepseek-v4-pro`，Challenger = `qwen3.8-flash`，同一批脱敏样例，硬门禁（编造、越权、跨租户串数据）必须为 0。
- 兜底（2026-09-17 定）：到 2026Q4 结束仍未出现低风险批量能力，本通道视为**永久关闭**（代码与 Eval 保留不删，零成本），不为「开过一次」而开。
- 最后更新日期：2026-09-17
