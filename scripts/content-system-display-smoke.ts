...
const standardStyleOutput = [
  ...
  "先核实真实信息。",
  "## 五、剪辑EDL文件",
  "0-3秒字幕：先别急着买工具。六、发布标题话题 标题：老板别先买工具。",
  "## 七、最佳发布时间",
  "待按账号数据验证。",
  "## 八、评论区引导话术",
  "评论区写流程。",
...
assert.deepEqual(delivery.sections.map((section) => section.title), [
  "选题策划", "口播逐字稿", "拍摄脚本", "拍摄注意事项", "剪辑EDL文件",
...
assert.ok(delivery.sections[5]?.lines.some((line) => line.includes("老板别先买工具")), "行尾拼接的第六项必须被恢复为发布标题与话题标签");
console.log("CONTENT_SYSTEM_DISPLAY_SMOKE_OK sections=9 duplicate_topic_section=0");
