...
function main(): void {
  ...
  assert.equal(isFixedPriceSku("livescript"), true, "livescript 必须被识别为固定价 SKU");
  assert.equal(isFixedPriceSku("meiye__livescript"), true, "美业专区的 livescript 同样固定价");
  ...
  for (const [zoneKey, industry] of Object.entries(v3Raw.industries)) {
    const overridePpu = industry.ov?.["livescript"]?.ppu;
    if (typeof overridePpu === "number") {
      assert.equal(overridePpu, 200, `${zoneKey} 专区不得用 ov 覆盖 livescript 价格（现为 ${overridePpu}）`);
    }
  }
  ...
  assert.ok(liveScriptSeeds.length >= 2, `直播话术必须同时在多个专区上架（got ${liveScriptSeeds.length}）`);
  for (const seed of liveScriptSeeds) {
    assert.equal(seed.ppu, 200, `货架 SKU ${seed.skuCode} 的 ppu 必须是 200`);
    assert.equal(isFixedPriceSku(seed.skuCode), true, `货架 SKU ${seed.skuCode} 必须命中固定价名单`);
  }
