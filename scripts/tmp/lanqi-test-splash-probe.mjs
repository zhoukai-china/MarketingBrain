#!/usr/bin/env node
/**
 * 「点任一功能就出现中间页」证据探针（真实 Chromium，只读页面、不写业务数据）。
 *
 * 现象：内测实例 https://api.lcppch.top/lanqi-test 上，首屏建立体验会话后，
 * 点侧栏任一功能（整页跳转）都会重新渲染
 * `兰琪美业 · 内测实例 / 正在进入体验工作区 / 正在进入美业智能体体验工作区…` 的中间页。
 *
 * 断言：
 *   1) 首屏（尚无本地会话）允许出现一次中间页 —— 只记录，不作断言；
 *   2) 已持有可用会话后，点侧栏导航**不得**再出现中间页；
 *   3) 已持有可用会话后，直接打开/刷新子页**不得**出现中间页；
 *   4) 三个场景最终都要真正落到兰琪页面。
 *
 * 用法：
 *   node scripts/tmp/lanqi-test-splash-probe.mjs --base https://api.lcppch.top/lanqi-test
 */
...
const outDir = argValue("--out", path.join(tmpdir(), "lanqi-test-splash-probe"));
const port = Number(argValue("--port", "9361"));

/** 中间页（DirectTestLoginGate）的三个文案标记。 */
const SPLASH_MARKERS = ["兰琪美业 · 内测实例", "正在进入体验工作区", "正在进入美业智能体体验工作区", "体验入口暂时打不开"];
...
async function sampleUntil(root, sessionId, { durationMs, readyText }) {
    ...
    try {
      snapshot = await evaluate(
        ...
        `(() => {
          ...
          const h2 = [...document.querySelectorAll("h2")].map((el) => el.innerText.trim()).join("|");
          return { path: location.pathname, text: text.slice(0, 400), h2 };
        })()`,
      );
    } catch { /* 导航切换执行上下文，跳过这一帧 */ }
    if (snapshot) {
  ...
  return { splashSeen, splashMs, splashText, readyMs, lastPath, lastText, elapsedMs: Date.now() - started };
}
...
async function runViewport(root, viewport) {
  ...
  info(`${viewport.label} 首屏（无会话）中间页`, `出现=${first.splashSeen}${first.splashMs !== null ? ` @${first.splashMs}ms` : ""} · 落地=${first.lastPath} · 就绪=${first.readyMs}ms`);
  record(`${viewport.label} 首屏最终落到兰琪页面`, first.readyMs !== null && first.lastPath.includes("/lanqi/"), `path=${first.lastPath} text=${first.lastText}`);
  ...
  record(
    ...
    `click=${clickResult} splash=${afterClick.splashSeen}${afterClick.splashMs !== null ? ` @${afterClick.splashMs}ms` : ""}${afterClick.splashText ? ` text=${afterClick.splashText}` : ""} · 落地=${afterClick.lastPath}`,
  );
  record(`${viewport.label} 点侧栏导航后落到公域获客页`, afterClick.lastPath.includes("/lanqi/acquire"), `path=${afterClick.lastPath}`);
  ...
  record(
    ...
    `splash=${reload.splashSeen}${reload.splashMs !== null ? ` @${reload.splashMs}ms` : ""}${reload.splashText ? ` text=${reload.splashText}` : ""} · 落地=${reload.lastPath}`,
  );
...
async function main() {
  ...
  console.log(`\nlanqi_test_splash_probe: ${failures === 0 ? "PASS" : "FAIL"} (${failures} failed)`);
